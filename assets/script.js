$( document ).ready(function() {
    ...
    function fetchData() {
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(response.reverse(), function(index, data) {
                    let row = $('<tr>');
                    if($('#actionHead').is(':visible')){
                        ...
                        row.append($('<td>').html("").attr('style','display:none;'));
                    }
                    if($('#rfidHead').is(':visible')){
                        row.append($('<td>').text(data.rfid_tag));
                    }else{
                        row.append($('<td>').text("").attr('style','display:none;'));
                    }
    ...
    $('#passwordField input').on('input', function() {
        ...
        // Perform password verification here, then reveal administrator privilleges
        if (password === 'pass') {
            $('#passwordField').hide();
            $('#admin').hide();
            $('#adminOptions').show();
